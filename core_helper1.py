...
# 제목
title_label = tk.Label(main_frame, text="🍁 Maple Core Optimizer (완전탐색)", 
                      font=("Arial", 16, "bold"), bg="#f0f0f0", fg="#2E7D32")
...
tk.Label(target_frame, text="🎯 목표 스킬 목록 (공백, 탭, 콤마로 구분)", 
         font=("Arial", 10, "bold"), bg="#f0f0f0").pack(anchor=tk.W)
target_entry = tk.Entry(target_frame, width=80, font=("Arial", 10))
target_entry.pack(pady=5, fill=tk.X)

# 최대 탐색 코어 수 입력 섹션
max_cores_frame = tk.Frame(main_frame, bg="#f0f0f0")
max_cores_frame.pack(fill=tk.X, pady=5)

tk.Label(max_cores_frame, text="🔢 최대 탐색 코어 수", 
         font=("Arial", 10, "bold"), bg="#f0f0f0").pack(anchor=tk.W)
max_cores_entry = tk.Entry(max_cores_frame, width=10, font=("Arial", 10))
max_cores_entry.pack(pady=5, anchor=tk.W)
max_cores_entry.insert(0, "6")  # 기본값 설정

# 코어 목록 입력 섹션
core_frame = tk.Frame(main_frame, bg="#f0f0f0")
core_frame.pack(fill=tk.BOTH, expand=True, pady=5)

tk.Label(core_frame, text="🧩 코어 목록 (한 줄에 3개씩, 엑셀 복붙 가능)", 
         font=("Arial", 10, "bold"), bg="#f0f0f0").pack(anchor=tk.W)
core_text = scrolledtext.ScrolledText(core_frame, width=80, height=20, 
                                      font=("Arial", 10), wrap=tk.WORD)
core_text.pack(pady=5, fill=tk.BOTH, expand=True)

# 계산 버튼
calc_button = tk.Button(main_frame, text="🔍 최적 조합 찾기 (완전탐색)", 
...
help_label = tk.Label(main_frame, text=help_text, 
                     ...
                     justify=tk.LEFT)
help_label.pack(pady=10, fill=tk.X)
